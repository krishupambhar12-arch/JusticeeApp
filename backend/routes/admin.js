...
router.post("/doctors", auth, async (req, res) => {
  try {
    ...
    const { name, qualification, gender, joiningDate, attorneyCode } = req.body;
...
// UPDATE CODE/ATTORNEY
router.put("/doctors/:id", auth, async (req, res) => {
  try {
    if (req.userRole !== "Admin") {
      return res.status(403).json({ message: "Only admins can update attorneys" });
    }

    const { name, qualification, gender, joiningDate, attorneyCode } = req.body;
    const codeId = req.params.id;

    // Check if attorney code already exists (excluding current record)
    if (attorneyCode) {
    ...
    const updatedCode = await Code.findByIdAndUpdate(
      codeId,
      { name, qualification, gender, joiningDate, attorneyCode },
    ...
    res.json({
      ...
      doctor: {
        ...
        qualification: updatedCode.qualification,
        gender: updatedCode.gender,
        joiningDate: updatedCode.joiningDate,
        attorneyCode: updatedCode.attorneyCode
      }
    });
  } catch (error) {
...
// DELETE CODE/ATTORNEY
router.delete("/doctors/:id", auth, async (req, res) => {
  try {
    if (req.userRole !== "Admin") {
      return res.status(403).json({ message: "Only admins can delete attorneys" });
    }
